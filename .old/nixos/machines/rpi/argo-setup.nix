...
in
{
  options = {
    services.argo-setup = {
      enable = lib.mkEnableOption "Argo CD setup service";
    };
  };

  config = lib.mkIf config.services.argo-setup.enable {
    systemd.services.argo-setup = {
      ...
      requires = [ "k3s.service" "secret-loader.service" ];
      ...
      # Fix: Move script inside systemd.services.argo-setup
      script = ''
        ...
        if [ "$failure_count" -eq 3 ]; then
          ...
          echo -e "$failed_steps"
          echo "Deleting helm release configmap and restarting service..."
          kubectl -n argocd delete secrets sh.helm.release.v1.argocd.v1
          exit 1
